...
def perform_kernel_calc(iposeps2, jposeps2, jmass):
    ...
    dev_jmass = cl.Buffer(ctx, mf.READ_ONLY | mf.COPY_HOST_PTR,
                          hostbuf=jmass)
    dev_iacc = cl.Buffer(ctx, mf.WRITE_ONLY, size=4*ilen*iposeps2.dtype.itemsize)
    ...
    exec_evt = kernel(queue, global_size, local_size,
                      dev_iposeps2, dev_jposeps2, dev_jmass, dev_iacc,
    ...
    iacc = np.empty((ilen,4), dtype=iposeps2.dtype)

    exec_evt = cl.enqueue_read_buffer(queue, dev_iacc, iacc)
    exec_evt.wait()
...
def perform_calc(bi, bj):
    ...
    iacc = perform_kernel_calc(iposeps2, jposeps2, jmass)
